...
| 2 | 统一 Generator 抽象 | ✅ | `Generator`(handle) + `GeneratorImpl`(base) + `GeneratorImplRegistry`，§3.1 |
| 3 | CPU Generator | ✅ | `CPUGeneratorImpl`(mt19937_64) |
| 4 | CUDA Generator（建议） | ✅ | `CUDAGeneratorImpl`(Philox4_32 + offset) |
| 5 | 默认 Generator 管理 | ✅ | CPU 单例 + 每 CUDA device 独立，`std::once_flag` 惰性初始化 |
| 6 | 统一全局 seed 入口 | ✅ | `manual_seed` / `manual_seed_cuda` |
| 7 | 默认 + 显式两条路径 | ✅ | `ResolveGenerator(optional<Generator>, Device)` |
| 8 | State get/set | ✅ | byte blob + magic/version header + cross-magic reject |
...
```
                user code
                   ...
                   ▼
          ┌──────────────────┐
          │  Generator       │   PImpl 句柄；shared_ptr<GeneratorImpl>
          │  (handle, L18)   │   ManualSeed / Seed / GetState / SetState / device()
          └────────┬─────────┘
                   │ delegates
                   ▼
          ┌──────────────────┐
          │  GeneratorImpl   │   抽象基类；mutex_ + device_ + StateMagic()
          │  (abstract base) │   纯虚 SetCurrentSeed / CurrentSeed / Reseed / GetState / SetState
          └─┬─────────────┬──┘
            │             │
            ▼             ▼
   CPUGeneratorImpl   CUDAGeneratorImpl
   mt19937_64         Philox4_32 (vendored, BSD-3)
   magic 'CPUG'       magic 'CUDG' + 16-byte (seed+offset+initial_seed)

           ┌────────────────┐
           │ default_pool   │   per-Device 单例，std::once_flag
           │ manual_seed()  │   全局 seed 入口；同步 CPU + 全部 CUDA 默认
           └────────────────┘

           ResolveGenerator(optional<Generator>, Device)
              ↓
           kernel: dispatcher.Call<>(...) (UniformRandom / NormalRandom / DropoutForward / DropoutBackward)
```

外部接口完全不暴露 `mt19937`/curand/Philox 类型；后端实现可替换。
...
| 4 | `Module::Train(bool)` 递归**不**跳 `__pp_*` | 与 `StateDict()` 对齐跳过 | `StateDict()` 跳是为了序列化去重（PP chunk 别名）；`Train()` 是幂等 setter 无去重需求；跳过会让未来 PP 改别名时静默漏切 chunk 内 Dropout 的 eval 开关 |
| 5 | CPU random kernel 单线程 + mutex | 每线程 sub-stream + Philox jump-ahead | 赛题 §2.2 明确性能非目标；正确性 trap test (`ConsecutiveCallsAdvanceState`) 优先；后续可解（§7） |
| 6 | `nn::Dropout` 不带 `inplace` 参数 | 仿 `nn.Dropout(p, inplace=True)` | autograd 侧需要支持原位写，与现 `Function::Apply` 契约不兼容；out-of-place 多一份显存但语义同 PyTorch 默认 |
...
| Module 训练态 | `Train(bool)/Eval()/IsTraining()` | `train()/eval()/training` | 递归到所有子模块；`__pp_*` 处理见决策 §3.2 之 4 |
| Dropout | `nn::Dropout(p, gen)` + `nn::function::Dropout(...)` | `nn.Dropout(p, inplace=False)` + `nn.functional.dropout` | 语义等价；缺 `inplace`（决策 §3.2 之 6） |
...
| 5 | `Module.Train` 不跳 `__pp_*` | `StateDict()` 跳是为序列化去重；`Train()` 跳会让 PP 改别名时漏切 | 见决策 §3.2 之 4 |
